function findCafe(key) {
    ...
    const filteredCafes = cafes.filter(cafe => cafe.name.toLowerCase().includes(key.toLowerCase()));

    const results = filteredCafes.map(cafe => {
        const place = places.find(place => place.id === cafe.place_id);
        return { name: cafe.name, ...place };
    });

    return results;
}

const res = findCafe('Avenue');
console.log(res);
